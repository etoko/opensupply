...
from .suppliers import supplier_first
...
#@view_config(route_name='home', renderer='index.mako')
def index(request):
    userid = 'me' #authenticated_userid(request)
...
#            return HTTPFound(location = came_from, headers = headers)
       ...
       # DO NOT FORGET TO UNCHECK not
        if not User.check_password(login, password):
            headers = remember(request, login)
#            headers.remember(request, login)
            #request.session.flash(u'Logged in successfully')
            print(u'Logged in successfully#########################')
            #return HTTPFound(location = came_from, headers = headers)
...
def modem_page(request):
    from opensupply.pygsm import GsmModem
    network = None
